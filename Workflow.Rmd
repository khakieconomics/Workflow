...
and so the variance $\sigma^{2}$ and so on. Yet in general we want to replace the parameters of parametric 
...
To gage which proposed density is a good one, we would like a function that gives a higher value for 
...
result in a "better" fit than some other unknowns. Likelihood functions are one approach. Under 
the assumption that each observation in X is independent of the others, they give us the probability
of observing the data given the parameters. This is most easily motivated with an example. 
...
and none have defaulted. Now let's come up with a proposed probability of default
