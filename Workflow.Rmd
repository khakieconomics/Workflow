...
What did we ust do? First, we created a new variable `defaulted`, which is 1 when the `loan_status` variable does
not contain the words "Charged Off", and 0 otherwise. The reason we turn it into a numeric is that we can now calculate
...
mini_loans %>% 
  ...
  geom_linerange(aes(ymin = proportion_defaulted - 2*standard_error, ymax = proportion_defaulted + 2*standard_error), colour = "green") +
...
the distribution of the the variable on the x-axis, and, by overlaying a non-linear regression curve, 
...
it might be a projected spending pattern (even though this is not "known", for a modeling exercise it might be assumed to be
known). In some cases, especially in time series and computer vision, the set of "knowns" might be empty.
We often use $X$ for "knowns". 
...
down the unknowns, we say the model is *identified*. This language can be misused in economics, where the term "identification"
...
$$
we read this as "each observation $i$ of the the random variable $y$ is distributed normally with location $\mu$ and scale $\sigma$". Given this information, we can calculate any statistic we want about the distribution 
